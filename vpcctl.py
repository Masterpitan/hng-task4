...
class VPCManager:
    ...
    def peer_vpcs(self, vpc1_name, vpc2_name):
        ...
        try:
            # Clean up existing peering interfaces and routes
            print(f"[INFO] Cleaning up existing peering interfaces")
            # Only delete one end of veth pair - Linux automatically removes both
            self.run_cmd(f"ip link delete {peer_veth1}", check=False)
            
            # Remove existing routes
            self.run_cmd(f"ip route del {vpc2_config['cidr']} via {vpc1_config['gateway']} dev {vpc1_config['bridge']}", check=False)
            self.run_cmd(f"ip route del {vpc1_config['cidr']} via {vpc2_config['gateway']} dev {vpc2_config['bridge']}", check=False)
            ...
            # Add routes for cross-VPC communication - route to each VPC's own gateway
            self.run_cmd(f"ip route add {vpc2_config['cidr']} via {vpc1_config['gateway']} dev {vpc1_config['bridge']}")
            self.run_cmd(f"ip route add {vpc1_config['cidr']} via {vpc2_config['gateway']} dev {vpc2_config['bridge']}")

            # Add routes to namespaces for proper isolation
            for subnet_name, subnet_info in vpc1_config["subnets"].items():
                ns_name = subnet_info["namespace"]
                # Remove existing route first
                self.run_cmd(f"ip netns exec {ns_name} ip route del {vpc2_config['cidr']} via {vpc1_config['gateway']}", check=False)
                self.run_cmd(f"ip netns exec {ns_name} ip route add {vpc2_config['cidr']} via {vpc1_config['gateway']}")
            
            for subnet_name, subnet_info in vpc2_config["subnets"].items():
                ns_name = subnet_info["namespace"]
                # Remove existing route first
                self.run_cmd(f"ip netns exec {ns_name} ip route del {vpc1_config['cidr']} via {vpc2_config['gateway']}", check=False)
                self.run_cmd(f"ip netns exec {ns_name} ip route add {vpc1_config['cidr']} via {vpc2_config['gateway']}")
    ...
    def unpeer_vpcs(self, vpc1_name, vpc2_name):
        ...
        print(f"\n=== Removing VPC peering: {vpc1_name} <-> {vpc2_name} ===")
        ...
        # Create veth pair names for peering
        peer_veth1 = f"p1-{self._short_name(vpc1_name, 5)}{self._short_name(vpc2_name, 5)}"
        peer_veth2 = f"p2-{self._short_name(vpc2_name, 5)}{self._short_name(vpc1_name, 5)}"

        try:
            # Remove peering interfaces - only delete one end
            self.run_cmd(f"ip link delete {peer_veth1}", check=False)
            ...
            print(f"[OK] VPC peering removed between {vpc1_name} and {vpc2_name}")
            return True

        except Exception as e:
            print(f"[ERR] Failed to remove peering: {e}")
            return False
...
def main():
    ...
    # Remove VPC peering
    unpeer_parser = subparsers.add_parser('unpeer-vpcs', help='Remove VPC peering')
    unpeer_parser.add_argument('vpc1', help='First VPC name')
    unpeer_parser.add_argument('vpc2', help='Second VPC name')
    ...
    if args.command == 'create-vpc':
        ...
        vpc_manager.unpeer_vpcs(args.vpc1, args.vpc2)
